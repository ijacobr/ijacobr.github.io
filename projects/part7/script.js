document.addEventListener("DOMContentLoaded", function () {
        ...
        sortButtons.forEach((button) => {
            button.addEventListener("click", function () {
                ...
                const cardItems = Array.from(
                    cardGallery.querySelectorAll(".card-item")
                );
                cardItems.sort((a, b) => {
                    const aValue =
                        parseInt(a.getAttribute("data-" + sortMetric)) || 0;
                    const bValue =
                        parseInt(b.getAttribute("data-" + sortMetric)) || 0;
    ...
    if (cardGallery) {
        cardGallery.addEventListener("click", function (e) {
            if (e.target.classList.contains("add-to-deck")) {
                ...
                saveDecks();
                alert(
                    `${cardName} added to deck: ${decks[selectedDeckIndex].name}`
                );
            }
        });
    }
    ...
    if (analysisDeckSelect && analyzeDeckBtn && analysisDeckList) {
        ...
        `;
            ...
            const avgAttack = cardCount
                ? (totalAttack / cardCount).toFixed(1)
                : 0;
            const avgHealth = cardCount
                ? (totalHealth / cardCount).toFixed(1)
                : 0;
            ...
            if (cardCount === 0) {
                ...
                if (avgCost < 4) {
                    analysisPoints.push(
                        "This deck is a low-cost, aggressive build, great at early pressure. It typically excels against slower, late-game strategies but can struggle if the opponent stabilizes."
                    );
                } else if (avgCost < 7) {
                    analysisPoints.push(
                        "This deck is a balanced midrange build, effective in both early and late stages. It can handle fast decks decently while still pressuring slower control decks."
                    );
                } else {
                    analysisPoints.push(
                        "This deck is a high-cost control build, relying on powerful late-game minions. It's strong against midrange or other slow decks, but may be vulnerable to hyper-aggro."
                    );
                }
                if (avgAttack >= 7) {
                    analysisPoints.push(
                        "Your deck's high average Attack suggests it can deliver strong burst damage, dominating trades and pressuring life totals."
                    );
                } else if (avgAttack <= 3 && avgAttack > 0) {
                    analysisPoints.push(
                        "Your deck's low average Attack indicates it may rely on other win conditions or spells to close out games."
                    );
                }
                if (avgHealth >= 7) {
                    analysisPoints.push(
                        "Your minions have high average Health, making them resilient to common removal and favorable in board trades."
                    );
                } else if (avgHealth <= 3 && avgHealth > 0) {
                    analysisPoints.push(
                        "Your minions have relatively low Health, so area-of-effect spells and efficient enemy trades could be problematic."
                    );
                }
                const attackHealthDiff = avgAttack - avgHealth;
                if (attackHealthDiff >= 3) {
                    analysisPoints.push(
                        "This deck leans heavily on offense, so you might struggle to keep minions alive. Consider adding more defensive options."
                    );
                } else if (attackHealthDiff <= -3) {
                    analysisPoints.push(
                        "This deck is very defensive, which can prolong the game but may lack finishing power if you can't pressure the opponent."
                    );
                }
            }
    ...
    /************************************************************
     * 6) CONTACT US FORM - SUBMIT EMAIL USING WEB3FORMS
     ************************************************************/
    ...
    if (contactForm) {
        ...
        contactForm.addEventListener("submit", async function (e) {
            ...
            const formData = {
                access_key: "a239d4d1-21e2-4df3-aba2-03dfcb01a861",
